...
%   Copyright 2016 - 2016 The MathWorks, Inc.
   ...
   doc = Document('Report', 'docx', 'template');
   ...
   while ~strcmp(currentHoleId, '#end#')
      ...
      switch currentHoleId 
         ...
         case 'AdvancedTable' 
          ...
          case 'Numberlab'
              processNumberlab(doc,reportData,currentHoleId);
          case 'Subjectname'
              processNumberlab(doc,reportData,currentHoleId);
         otherwise
...
function processStandardHole(doc, reportData, holeId)
   ...
   t = Text(data,'AR_Normal');   
   append(doc, t);
end

function processNumberlab(doc, reportData, holeId)
   import mlreportgen.dom.*
   ...
   %is a mismatch between the hole-names in the template and the field-
   %names of the reportData-structure
%    Эта функция заполняет все отверстия, которые не требуют специальной обработки.
% Блок try / catch выполняет обработку ошибок в случае, если
% несоответствие между именами отверстий в шаблоне и полем
%   имена reportData-структуры
   try
      data = reportData.(holeId);
   catch
      warning('Undefined Hole-Id: %s', holeId);
