...
_readAdc:
; .FSTART _readAdc
; 0000 000E // Запуск преобразования
; 0000 000F ADCSRA |=    _BV(ADSC);          //_BV(ADFR)
	SBI  0x6,6
; 0000 0010 // Ожидание завершения преобразования
; 0000 0011 while(BIT_IS_CLEAR(ADCSRA, ADIF));
_0x13:
	SBIS 0x6,4
	RJMP _0x13
; 0000 0012 // Сброс флага прерывания АЦП
; 0000 0013 ADCSRA |= _BV(ADIF);
	SBI  0x6,4
; 0000 0014 return ADCL+(ADCH<<8);
	IN   R30,0x4
	LDI  R31,0
	MOVW R26,R30
	IN   R30,0x5
...
; .FSTART _main
; 0000 0019     int val;
; 0000 001A     init_segments();
;	val -> R16,R17
	RCALL _init_segments
; 0000 001B     ADMUX = _BV(REFS0) |  3 ;  // Выбор канала
	LDI  R30,LOW(67)
	OUT  0x7,R30
; 0000 001C     ADCSRA |=   _BV(ADEN)  ;//инициализация АЦП
	SBI  0x6,7
; 0000 001D 
; 0000 001E     // Инициализация портов ввода/вывода
; 0000 001F     DDRE = _BV(4) | _BV(5);
	LDI  R30,LOW(48)
	OUT  0x2,R30
; 0000 0020 
; 0000 0021     // Инициализация таймера №3
; 0000 0022     // 8-ми разр. быстрая ШИМ WGM
; 0000 0023     //COM3B Сброс OCnX при совпадении (установка 0)     GREEN
; 0000 0024     // COM3C установка OCnX при совпадении (установка 1)  RED
; 0000 0025     TCCR3A =  _BV(COM3B1) | _BV(COM3C1) | _BV(COM3C0) | _BV(WGM31) | _BV(WGM30);
	LDI  R30,LOW(47)
	STS  139,R30
; 0000 0026     TCCR3B = _BV(WGM32) | _BV(CS30);
	LDI  R30,LOW(9)
	STS  138,R30
; 0000 0027 
; 0000 0028 while (1)
_0x16:
; 0000 0029     {
; 0000 002A          val=readAdc();
	RCALL _readAdc
	MOVW R16,R30
; 0000 002B          indic_int(val);
	MOVW R26,R16
	RCALL _indic_int
; 0000 002C          OCR3CH = high(val);
	MOVW R30,R16
	CALL __ASRW8
	STS  131,R30
; 0000 002D          OCR3CL = low(val);
	STS  130,R16
; 0000 002E          OCR3BH = high(val) ;
	MOVW R30,R16
	CALL __ASRW8
	STS  133,R30
; 0000 002F          OCR3BL = low(val) ;
	STS  132,R16
; 0000 0030          delay_ms(20);
	LDI  R26,LOW(20)
	LDI  R27,0
	CALL _delay_ms
; 0000 0031     }
