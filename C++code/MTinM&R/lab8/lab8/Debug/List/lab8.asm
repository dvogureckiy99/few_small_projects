...
_main:
; .FSTART _main
; 0000 005F     int field; // Значение с датчика Холла
; 0000 0060     unsigned long ref; // Задание
; 0000 0061     int error = 0; // Ошибка управления
; 0000 0062     int control; // Сигнал управления
; 0000 0063     int error_last=0; //   Ошибка управления последнее
...
; 0000 0065     #asm("sei");  //разрешение прерываний
	sei
; 0000 0066     EICRA |= _BV(ISC11) | _BV(ISC10) ;//на фронт
	LDS  R30,106
	ORI  R30,LOW(0xC)
	STS  106,R30
; 0000 0067     EIMSK |= _BV(INT1);
	...
	ORI  R30,2
	OUT  0x39,R30
; 0000 0068     // Инициализация портов ввода/вывода
; 0000 0069     DDRB |= _BV(5) ;//ШИМ управления
	SBI  0x17,5
; 0000 006A     DDRG |= _BV(3);//направление тока
	LDS  R30,100
	ORI  R30,8
	STS  100,R30
; 0000 006B     DDRD &= ~_BV(2); //кнопка
	CBI  0x11,2
; 0000 006C     //прерывание кнопки
...
; 0000 0079         if(flag_control)
	...
	RJMP _0x2F
; 0000 007A         {
; 0000 007B             //управления
; 0000 007C             // Получение и фильтрация сигнала с датчика
; 0000 007D             // магнитного поля
...
; 0000 007F             readAdc(1) + readAdc(1) + readAdc(1)) / 5;
	...
	CALL __DIVD21U
	MOVW R16,R30
; 0000 0080             //field -= 512;   //если  показания датчика равны 512, то field равно 0
; 0000 0081 
; 0000 0082             ref = readAdc(3)*182;
	...
	CALL __MULD12U
	CALL SUBOPT_0x6
; 0000 0083             ref = ref/1023+508;  // Получение задания с ручки потенциомера
	__GETD2S 2
	__GETD1N 0x3FF
	CALL __DIVD21U
	__ADDD1N 508
	CALL SUBOPT_0x6
; 0000 0084             if(field<=2) //магнит лежит
	__CPWRN 16,17,3
	BRGE _0x30
; 0000 0085             {
; 0000 0086                 ref=559;//подбрасываем магнит, чтобы он нележал
	__GETD1N 0x22F
	CALL SUBOPT_0x6
; 0000 0087             }
; 0000 0088 
; 0000 0089             // Расчёт ошибки управления
...
_0x30:
	...
	SBC  R31,R17
	MOVW R18,R30
; 0000 008B             // Расчёт сигнала управления
...
; 0000 008D             error_last=error;
	__PUTWSR 18,19,0
; 0000 008E 
; 0000 008F             if(field>=460)  //прилипание, нужно "отпустить магнит"
	__CPWRN 16,17,460
	BRLT _0x31
; 0000 0090             {
...
; 0000 0093 
; 0000 0094 
; 0000 0095             // Ограничение сигнала управления
...
; 0000 009D             }
; 0000 009E 
; 0000 009F             // Установка сигнала управления
; 0000 00A0             if(control >= 0)
_0x33:
	TST  R21
	BRMI _0x34
; 0000 00A1             {
; 0000 00A2                 PORTG &= ~_BV(3); //установка положительного тока
	LDS  R30,101
	ANDI R30,0XF7
	STS  101,R30
...
_0x34:
; 0000 00A6             {
; 0000 00A7                 PORTG |= _BV(3); //установка отрицательного тока
	LDS  R30,101
	ORI  R30,8
	STS  101,R30
...
; 0000 00AA             indic_int( field );
	MOVW R26,R16
	RCALL _indic_int
; 0000 00AB         }else //настройка коэффиентов
	RJMP _0x36
_0x2F:
; 0000 00AC         {
