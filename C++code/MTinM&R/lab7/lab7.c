...
void SPI_init()
{    //MOSI Ч PORTG3 на выход 
     //MISO Ч PORTG1 на вход
     //SCLK Ч PORTG0 на вход
...
void initTimer()
{   // Ѕыстрый Ў»ћ,  сброс PORTB4 по совпаднию с OCR0
...
void main(void)
{
    ...
    initTimer(); 
    SPI_init(); 
    resetAD7798();//  сброс ј÷ѕ
    setAd7798();  //настройка на посто€нное измерение 
    while(1)
    { 
       data = (char)(readAD7798()>>16); //сохранение старшего байта измерени€                                                                    
       indic_5bit(data); 
       //включение звукового оповещни€ о первышении 16 
