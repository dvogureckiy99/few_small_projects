...
void LCD_message(char cmd,int type );
void clear_display()      ;
...
void LCD_data_string(int data, unsigned char i) ;
...
void main(void)
{
    ...
    unsigned char T=1;
    int i, y, u[5]={8, 28, 48, 68, 88};
    
    DDRC = 0xFF;            // все разряды PORTC на выход
    DDRD|= ((1<<E)|(1<<RS));// разряды PORTD на выход  
    
    LCD_init();    //инициализация
    clear_display(); //Очистить дисплей и установить курсор в нулевую позицию (адрес 0)
    LCD_string("РЕЗУЛЬТАТ:");
    ...
    {   
        ...
        for (i=0; i<5; i++)
        {
            y = y + T * (-5*y+u[i]/4-1); //расчет заданной функции
            LCD_data_string(y, i); //вывод результат строкой i - аргумент функции y - значение
            delay_ms(1000);
        }
    }  
...
{ 
    ...
    unsigned char k=0;
        ...
        while (k<5) 
        {
            ...
            if (k==2) LCD_message(i+'0',DATA); // вывод аргумента функции
            ...
            LCD_message(STR[k],DATA); //оформление
            k++;
            delay_ms(50);
